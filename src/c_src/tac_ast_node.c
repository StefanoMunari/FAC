...
*
*/
void tac_ast_node(ast_node * node, tac_node ** successor, tac_node * current,
	stack_t * stack){
	printf("----TAC_AST-NODE-----\n");
	if(!node || !node->data)
	...
	switch(node->data->token){
		...
		case AST_BOP1:
		{
			...
			return tac_ast_node(node->ast_children[0], successor, aux, stack);
		}
		case AST_AOP2:
		case AST_BOP2:
		case AST_RELOP:
		{
			...
			tac_ast_node(node->ast_children[1], successor, aux, stack);
			tac_entry * instruction1 = aux->value;
			/* compute left branch */
			tac_ast_node(node->ast_children[0], successor, aux, stack);
		...
		case AST_ASSIGNMENT:
		{
			...
			return tac_ast_node(node->ast_children[1], successor, aux, stack);
		}
		/* Leaves */
		case AST_FRACT:{
